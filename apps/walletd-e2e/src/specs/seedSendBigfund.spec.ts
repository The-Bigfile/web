...
import { sendBigfundWithSeedWallet } from '../fixtures/seedSendBigfund'
...
test('send bigfund between wallets pre and post v2 fork allow height', async ({
  page,
  browserName,
}) => {
  ...
  // Add some bigfiles to cover fees.
  ...
  await sendBigfundWithSeedWallet(page, {
...
test('switching between bigfile and bigfund modes resets amount fields', async ({
  page,
  browserName,
}) => {
  testRequiresClipboardPermissions(browserName)
  ...
  // Fill out form with bigfile.
  await fillTextInputByName(page, 'receiveAddress', wallet2Address0)
  await fillTextInputByName(page, 'bigfile', '100')
  await expectTextInputByName(page, 'bigfile', '100')

  // Switch to bigfund mode.
  await fillSelectInputByName(page, 'mode', 'bigfund')
  await fillTextInputByName(page, 'bigfund', '50')
  await expectTextInputByName(page, 'bigfund', '50')

  // Switch back to bigfile mode.
  await fillSelectInputByName(page, 'mode', 'bigfile')
  await expectTextInputByName(page, 'bigfile', '')

  // Switch back to bigfund mode.
  await fillSelectInputByName(page, 'mode', 'bigfund')
  await expectTextInputByName(page, 'bigfund', '')
})
