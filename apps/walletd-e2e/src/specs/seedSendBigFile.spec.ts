...
import { sendBigFileWithSeedWallet } from '../fixtures/seedSendBigFile'
...
test('send bigfile between wallets pre and post v2 fork allow height', async ({
  page,
  browserName,
}) => {
  ...
  await sendBigFileWithSeedWallet(page, {
  ...
  await expect(navbar.getByText(`${amountV1.toFixed(3)} BIG`)).toBeVisible({
    timeout: 20_000,
  })
  ...
  await navigateToWallet(page, wallet2Name)
  await expect(
    navbar.getByText(`${(amountV1 + amountV2).toFixed(3)} BIG`)
  ).toBeVisible({
    timeout: 20_000,
  })
})
